...
class EmployeeListVC: BaseViewController<EmployeeListVCRootView>  {
    ...
    override func viewDidLoad() {
        ...
        // вот это замыкание пусть останется просто чтобы было куда смотреть. Я сделал функцию из этого замыкания и она даже заработала, но не сказал бы, что понимаю почему это сработало (ну +- только если и то не факт что напишу аналогично сам)
